...
class Obj:
    ...
    def cleanup(obj):
        query_file_path = '/application/RPA/LRPU/PLSInvestorRulesAutomation/REQUIRED/MsgQuery.txt'
        # filename = 'data.xlsx'
        today = date.today()
        ...
        if len(processed_results) > 0:
            ...
            InputFile = 'BKFS Daily Investor Rules Message_' + datetime.now().strftime('%m.%d.%Y') + '.xlsx'
            df.to_excel('/m_drive/LRPU/BKFS Automation Projects (Live)/BKFS PLS Investor Rule/' + InputFile, index=False)
            ...
            wb = openpyxl.load_workbook('/m_drive/LRPU/BKFS Automation Projects (Live)/BKFS PLS Investor Rule/' + InputFile)
            ...
            wb.save('/m_drive/LRPU/BKFS Automation Projects (Live)/BKFS PLS Investor Rule/' + InputFile)
            ...
            wb.save('/m_drive/LRPU/BKFS Automation Projects (Live)/BKFS PLS Investor Rule/' + InputFile)
            file_path = '/m_drive/LRPU/BKFS Automation Projects (Live)/BKFS PLS Investor Rule/' + InputFile
            df.to_excel(file_path, index=False)
            obj.InputFile = file_path
    ...
    def sendOutlookMail(obj):
        ...
        if obj.SystemException:
            ...
            msg['Subject']=obj.config['PROCESSNAME'] + "  Message_" + datetime.now().strftime('%m.%d.%Y')
            BODY+=obj.config['ExchEmailBodyMsgSuccess'] + datetime.now().strftime('%m.%d.%Y')
