...
class Obj:
    ...
    def cleanup(obj):
        ...
        if len(processed_results) > 0:
            ...
            df.to_excel('/m_drive/LRPU/BKFS Automation Projects (Live)/BKFS PLS Investor Rule/Input/' + InputFile, index=False)
            ...
            wb = openpyxl.load_workbook('/m_drive/LRPU/BKFS Automation Projects (Live)/BKFS PLS Investor Rule/Input/' + InputFile)
            ...
            wb.save('/m_drive/LRPU/BKFS Automation Projects (Live)/BKFS PLS Investor Rule/Input/' + InputFile)
            ...
            wb.save('/m_drive/LRPU/BKFS Automation Projects (Live)/BKFS PLS Investor Rule/Input/' + InputFile)
            file_path = '/m_drive/LRPU/BKFS Automation Projects (Live)/BKFS PLS Investor Rule/Input/' + InputFile
    ...
    def sendOutlookMail(obj):
        ...
        if obj.SystemException:
            ...
            BODY+=obj.config['ExchEmailBodySuccess'] + datetime.now().strftime('%m.%d.%Y') + " as per attached input file".replace('AAAAA',str(obj.InputFile.split('/')[-1]))
...
if __name__=="__main__":
    ...
    Error:
    obj.sendOutlookMail()
  File "/application/RPA/LRPU/PLSInvestorRulesAutomation/SRC/Framework/Test email.py", line 213, in sendOutlookMail
    BODY+=obj.config['ExchEmailBodySuccess'] + datetime.now().strftime('%m.%d.%Y') + " as per attached input file".replace('AAAAA',str(obj.InputFile.split('/')[-1]))
